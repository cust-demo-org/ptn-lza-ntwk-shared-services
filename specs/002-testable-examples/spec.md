...
- **FR-007**: Each example MUST use the naming module (`Azure/naming/azurerm`) to generate unique resource names, ensuring parallel deployments do not collide.
...
- **FR-020** *(root module change)*: Root module variable descriptions MUST use multi-line heredoc format and reference the specific AVM module variable descriptions they map to, so consumers can trace each field back to the upstream AVM interface.
- **FR-021** *(root module change)*: All user-configurable AVM module variables MUST be passed through in the root module's variable type definitions. An audit of all 13 AVM modules must be performed to identify missing fields (e.g., `lock` and `role_assignments` are missing from the `resource_groups` variable type).
...
- **SC-005**: Every example directory has at minimum `terraform.tf`, `main.tf`, `variables.tf`, `terraform.tfvars`, and `_header.md`. No example directory contains only a `terraform.tfvars`.
