...
- What about `byo_private_dns_zones` which reference external Private DNS Zones? — The example deploys an `azurerm_private_dns_zone` inline to satisfy the reference.
...
- For features requiring external resources that are impractical to deploy inline (e.g., existing Private DNS Zones for the `byo_private_dns_zones` variable), examples will deploy a minimal `azurerm_private_dns_zone` inline.
