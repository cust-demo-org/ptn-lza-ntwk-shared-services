...
- [x] T009 [US1] Rewrite `byo_private_dns_zones` (starting ~L562) description in `variables.tf`: read AVM source from `examples/full/.terraform/modules/` private_dns_zone link submodule, follow contracts §1 (map(object) with "deliberately arbitrary"), §2, §4 (cross-reference keys: `virtual_network_key` references `virtual_networks` variable); ~5 attrs
