...
- **FR-027**: A global `enable_telemetry` variable (type `bool`, default `true`) MUST be added to the root module and passed through to all AVM module calls in `main.tf` that support it (12 of 14 modules — `private_dns_zone_virtual_network_link` and `vhub_vnet_connection` are submodules whose interfaces do not expose `enable_telemetry`). This variable controls whether AVM modules collect anonymous usage telemetry. The variable MUST also be added to all example `variables.tf` files and wired through each example's `main.tf` `module "pattern"` call.
- **FR-028**: Resources with globally unique names (Key Vault, Storage Account) MUST support per-resource random suffix configuration via `name_random_suffix_configuration` instead of a global `random_suffix_length` variable. Each resource's `name_random_suffix_configuration` is an optional object with a `length` field (number of random lowercase alphanumeric characters to append). Key Vault additionally supports `append_with_hyphen` (bool, default `true`) to control whether the suffix is separated by a hyphen. Storage Account appends directly (no separator — only lowercase alphanumeric allowed). A separate `random_string` resource is created per key vault and per storage account that has the configuration set. When `null` (default), the `name` is used as-is. Both resource names have a 24-character maximum — users are warned in the description.
- **FR-029**: Every `role_assignments` block MUST support an `assign_to_caller` attribute (`optional(bool, false)`) that, when `true`, automatically populates `principal_id` with `data.azurerm_client_config.current.object_id` (the identity running Terraform). `assign_to_caller` is mutually exclusive with `principal_id` and `managed_identity_key`. Validation blocks MUST enforce that exactly one of the three is set (using a sum-equals-1 expression). In `resource_groups` (which cannot use `managed_identity_key` due to circular dependency), `assign_to_caller` is mutually exclusive with `principal_id` only. The `main.tf` resolution logic MUST check `ra.assign_to_caller` first, then `ra.managed_identity_key`, then fall back to `ra.principal_id`. All example `variables.tf` files MUST mirror the type changes. The full example `terraform.tfvars` MUST demonstrate `assign_to_caller = true`.
- **FR-030**: Resources with `customer_managed_key` objects (`log_analytics_workspace_configuration`, `storage_accounts`) MUST support key-based references to pattern-managed Key Vaults, Key Vault keys, and managed identities. The `customer_managed_key` object MUST add `key_vault_key` (mutually exclusive with `key_vault_resource_id`), `key_key` (mutually exclusive with `key_name`, requires `key_vault_key`), and `user_assigned_identity.key` (mutually exclusive with `user_assigned_identity.resource_id`). All existing fields (`key_vault_resource_id`, `key_name`, `user_assigned_identity.resource_id`) MUST become `optional(string)`. Validation blocks MUST enforce mutual exclusivity and that `key_key` requires `key_vault_key`. In `main.tf`, resolution MUST use `local.key_vault_resource_ids` for `key_vault_key`, the key vault's `keys` map `name` attribute for `key_key`, and `local.managed_identity_resource_ids` for `user_assigned_identity.key`. New locals MUST be added for `key_vault_resource_ids` and `managed_identity_resource_ids`. All example `variables.tf` files MUST mirror the type changes. The full example `terraform.tfvars` MUST demonstrate key-based CMK referencing.
- **FR-031**: Resources with inline `managed_identities` objects (`storage_accounts`) MUST support `user_assigned_keys` (`optional(set(string), [])`) to reference managed identities created by the pattern via their map keys. `user_assigned_keys` is NOT mutually exclusive with `user_assigned_resource_ids` — the resolved resource IDs are merged (appended) using `setunion`. In `main.tf`, resolution MUST use `local.managed_identity_resource_ids` to convert keys to resource IDs and merge with the explicit `user_assigned_resource_ids`. All example `variables.tf` files MUST mirror the type changes. The full example `terraform.tfvars` MUST demonstrate `user_assigned_keys`.
- **FR-032**: The pattern MUST support `backup_vaults` — a `map(object({}))` variable (default `{}`) that creates Azure Data Protection Backup Vaults using AVM module `Azure/avm-res-dataprotection-backupvault/azurerm` v2.0.3. Each vault MUST link to the pattern via: `resource_group_key` → `local.resource_group_names`, `location` defaulting to `var.location`, tags merged with `var.tags`, `enable_telemetry` passthrough, `role_assignments` with 3-way resolution (`assign_to_caller`/`managed_identity_key`/`principal_id`), `diagnostic_settings` with `use_default_log_analytics`, `managed_identities` with `user_assigned_keys` via `setunion`, `customer_managed_key` with `key_vault_key`/`key_key`/`user_assigned_identity.key` key-based references, and `lock` passthrough. Validation blocks MUST enforce role assignment XOR and CMK mutual exclusivity. Backup policies and instances are passed through to the module. Output MUST expose `resource_id` and `name` per vault.
- **FR-033**: The pattern MUST support `recovery_services_vaults` — a `map(object({}))` variable (default `{}`) that creates Azure Recovery Services Vaults using AVM module `Azure/avm-res-recoveryservices-vault/azurerm` v0.3.3. Each vault MUST link to the pattern via: `resource_group_key` → `local.resource_group_names`, `location` defaulting to `var.location`, tags merged with `var.tags`, `enable_telemetry` passthrough, `role_assignments` with 3-way resolution, `private_endpoints` with `vnet_key`/`subnet_key` and DNS zone `keys` resolution, `diagnostic_settings` with `use_default_log_analytics`, `managed_identities` with `user_assigned_keys` via `setunion`, `customer_managed_key` with key-based references, and `lock` passthrough. VM/file-share/workload backup policies and protected items are passed through. Validation blocks MUST enforce role assignment XOR and CMK mutual exclusivity. Output MUST expose `resource_id` per vault.
- **FR-034**: In `main.tf`, every `role_assignments` block that supports `managed_identity_key` MUST auto-resolve `principal_type` to `"ServicePrincipal"` when `managed_identity_key` is not null, falling back to the user-supplied `ra.principal_type` otherwise. The standalone `module "role_assignment"` MUST apply the same logic. The `resource_groups` role_assignments block (which has no `managed_identity_key`) MUST remain unchanged. In full example `terraform.tfvars`, every `assign_to_caller` entry MUST include `principal_type = "User"` with an inline comment explaining the assumption.
