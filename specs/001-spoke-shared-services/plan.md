...
| avm-res-network-privatednszone | 0.5.0 | `0.5.0` | Private DNS Zones (optional, via `private_dns_zones`) |
...
| III | Configuration-Driven Reusability | **PASS** | All configuration via `terraform.tfvars`. Variables have explicit types (no `any`), descriptions, and secure defaults. Implicit null/empty-map toggles for optional components (`bastion_hosts`, `flowlog_configuration`). Multiple instantiations supported via consumer-provided names. See [contracts/variables.md](contracts/variables.md). |
...
| VII | Operability & Observability | **PASS** | All resource IDs, names, and connection-relevant attributes exported as outputs. All outputs have descriptions. Consumer-managed CAF naming. Common `tags` variable. Log Analytics workspace ID exported. See [contracts/outputs.md](contracts/outputs.md). |
...
├── locals.tf            # Computed locals (log analytics workspace ID, resource references)
...
**Structure Decision**: Flat root module — AVM modules provide the sub-module abstraction. Standard Terraform file convention (`main.tf`, `variables.tf`, `outputs.tf`, `terraform.tf`) plus `locals.tf` for computed values. A `locals.tf` is added because the pattern requires computed values (log analytics workspace resolution, resource reference lookups) that are better isolated from `main.tf`. Example `.tfvars` files demonstrate the three primary use cases.
...
| private dns zone | `domain_name`, `resource_group_name`, `virtual_network_links`, `tags` | `lock`, `role_assignments` |
...
|---|---|---|---|---|
| `private_dns_virtual_network_link` | `avm-res-network-privatednszone` | 0.5.0 | When `byo_private_dns_zones` is non-empty | `parent_id` (DNS zone ID), `virtual_network_id`, `registration_enabled`, `resolution_policy` |
...
1. Resource groups
2. Log Analytics workspace (auto-create, or skip if external ID provided)
...
8. BYO Private DNS zone VNet links (via `private_dns_virtual_network_link` submodule, from `byo_private_dns_zones`)
...
11. Bastion host (optional, requires `AzureBastionSubnet` from step 5)
