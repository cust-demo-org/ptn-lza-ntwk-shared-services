...
1. **Given** a `terraform.tfvars` file specifying a spoke VNet with one subnet and a hub VNet peering configuration in the VNet's `peerings` map, **When** `terraform apply` is executed, **Then** the spoke VNet, subnet, NSG (user-defined rules), route table (user-defined routes), VNet peering (both directions), and diagnostic settings are created with no errors.
...
A platform engineer deploys a spoke VNet connected to an Azure Virtual WAN (vWAN) hub instead of a traditional hub VNet. They provide the Virtual Hub resource ID and VNet reference in the `vhub_connectivity_definitions` map in `terraform.tfvars`.
...
1. **Given** a `terraform.tfvars` specifying a `vhub_connectivity_definitions` entry with a Virtual Hub resource ID and a VNet reference, **When** `terraform apply` is executed, **Then** a Virtual Hub VNet connection is created linking the spoke to the vWAN hub.
2. **Given** a VNet has both hub peering (via `peerings` map) and a vWAN connection (via `vhub_connectivity_definitions`), **When** `terraform apply` is executed, **Then** both connectivity mechanisms are created independently — they are not mutually exclusive.
...
- What happens when no peering or vWAN connectivity is configured? When the `peerings` map inside a VNet definition is empty (or omitted) and `vhub_connectivity_definitions` is empty, no peering or vWAN connection resources are created; the spoke VNet exists in isolation.
...
- **FR-007**: MUST support associating an NSG with every subnet (unless the subnet type prohibits it, e.g., `AzureBastionSubnet` has specific NSG requirements managed by Bastion AVM).
...
- **FR-009**: The pattern MUST provision route tables with configurable routes. The pattern MUST NOT auto-inject any routes; all routes (including the default route) are user-defined via the `routes` map. When hub VNet peering is configured (via the VNet module's `peerings` map), the consumer MUST add a `0.0.0.0/0` route pointing to the hub firewall/NVA IP address to force traffic through the hub. When vWAN connectivity is configured (via `vhub_connectivity_definitions`), a user-defined default route is NOT required — the Virtual WAN hub automatically propagates the default route to connected spokes when `internet_security_enabled = true`.
- **FR-010**: Route table association to subnets MUST be configurable via input variables.

#### Connectivity — Hub VNet Peering

- **FR-011**: Hub VNet peering MUST be configured via the AVM VNet module's `peerings` map within each VNet definition. This allows per-VNet peering configuration — each VNet can peer to a different hub or have no peering. The `peerings` map follows the AVM `avm-res-network-virtualnetwork` module interface.
- **FR-012**: The AVM VNet module's peering interface supports `create_reverse_peering` (default: `false`) to control whether the hub-to-spoke reverse peering is also created. When reverse peering is enabled, the deployer MUST have write access to the hub VNet.
...
- **FR-017**: The pattern MUST support provisioning one or more Azure Key Vaults. Public network access MUST default to disabled and network ACLs MUST default to `Deny` (secure-by-default).
...
- **FR-023**: The resource reference pattern distinguishes two cases:
  ...
  - **External references** (resources created outside this pattern, e.g., existing Private DNS Zones, hub VNets, Virtual Hubs): MUST be referenced by explicit Azure resource ID only, since these resources are not in this Terraform context and cannot be resolved by key.
...
- **FR-027**: Diagnostic settings MUST be enabled for every resource that supports them. Logs MUST be sent to a Log Analytics workspace. Exception: the auto-created Log Analytics workspace itself is exempt from diagnostic settings (no self-referencing diagnostics).
...
- **FR-033**: Terraform input/output documentation MUST be generated by `terraform-docs` using the `"markdown document"` formatter. The `.terraform-docs.yml` config MUST reference `_header.md` (pattern overview, features, usage, quickstart) and `_footer.md` (contributing guide, SpecKit attribution) via `header-from` and `footer-from`. AI-generated or manually curated input/output tables are forbidden.
- **FR-034**: Each module directory MUST contain a `README.md` with `<!-- BEGIN_TF_DOCS -->` / `<!-- END_TF_DOCS -->` injection markers. The entire README content between the markers MUST be auto-generated by `terraform-docs` from `_header.md` + auto-generated docs + `_footer.md`. No manual content between the markers is permitted.
- **FR-035**: A quality gate MUST verify `terraform-docs` output is up-to-date by running `terraform-docs markdown document . --output-check`; stale docs MUST fail the gate.

#### Validation & Quality Gates

- **FR-036**: The following validation gates MUST pass before `terraform apply`: `terraform fmt -check -recursive`, `terraform validate`, and `terraform plan`. Note: `terraform plan` is expected to fail in local environments for pattern modules that omit provider blocks (providers are configured by the consumer). The `terraform plan` gate is meaningful only in a CI environment where a consumer wrapper provides the provider configuration.
...
- **FR-039**: For every AVM module consumed, variable names and complex object shapes MUST be verified against the actual module source (`terraform init` → read `.terraform/modules/<name>/variables.tf` and `outputs.tf`). Registry pages and READMEs MAY lag behind the actual interface. If a variable is not confirmed from the source, it MUST be marked `TBD` — never invented. AVM submodules MUST be treated as distinct integration points — they follow different conventions from root modules (e.g., `parent_id` instead of `resource_group_name`, explicit `name` arguments, different output shapes).
...
- **FR-041**: For single-instance optional features (e.g., Bastion, Network Watcher), the pattern MUST pass through the upstream AVM module's full variable interface rather than wrapping or cherry-picking a subset. This ensures zero-maintenance alignment with upstream module updates. For multi-instance features (e.g., Key Vaults, NSGs), the pattern MAY curate the exposed interface where the AVM module's full interface would create excessive complexity across many instances.

#### Validation Strategy

- **FR-042**: Input validations that reference a single variable MUST be implemented as `variable` validation blocks. Validations that reference multiple variables MUST be implemented as `lifecycle.precondition` blocks on a `terraform_data` resource, since Terraform does not support `lifecycle` blocks inside `module` blocks.
- **FR-043**: Optional features that produce exactly zero or one instance (singletons) MUST use `count` with a ternary conditional. Optional features that produce zero-to-many instances (collections) MUST use `for_each` over a map variable with an empty-map default. This distinction ensures clean output expressions and predictable state addressing.
...
- **Network Watcher (Flow Logs)**: Manages VNet flow logs on an existing Azure Network Watcher. Attributes: network watcher ID, network watcher name, resource group, flow logs (target resource ID, storage account ID, retention policy, traffic analytics). Provisioned via AVM module `avm-res-network-networkwatcher`.
...
- **SC-007**: Switching from hub VNet peering to vWAN (or adding both) requires changing only input variables (`virtual_networks[*].peerings` and `vhub_connectivity_definitions`), not `.tf` source code.
...
*Captured during the 001-spoke-shared-services implementation (March 2026). Design and technical learnings have been encoded into functional requirements above. The process insights below are retained for future spec authors.*
