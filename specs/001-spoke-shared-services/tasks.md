...
- [X] T002 [P] Create terraform-docs configuration in .terraform-docs.yml — configure `formatter: "markdown document"` output format with `_header.md` and `_footer.md` content injection, sort by required/type, enable full README.md generation between `<!-- BEGIN_TF_DOCS -->` / `<!-- END_TF_DOCS -->` markers via content template including `{{ .Header }}`, `{{ .Requirements }}`, `{{ .Resources }}`, `{{ .Inputs }}`, `{{ .Outputs }}`, `{{ .Modules }}`, and `{{ .Footer }}`
- [X] T003 [P] Create tflint configuration in .tflint.hcl — enable the azurerm ruleset plugin with recommended rules
- [X] T004 [P] Create _header.md with pattern overview (features, usage examples table, quickstart), _footer.md with contributing guide (pattern conventions, SpecKit workflow, attribution), and README.md with `<!-- BEGIN_TF_DOCS -->` / `<!-- END_TF_DOCS -->` markers per FR-034 — README content is fully auto-generated by terraform-docs from _header.md + auto-docs + _footer.md
...
Task T004: Create _header.md, _footer.md, and README.md with terraform-docs markers
```
